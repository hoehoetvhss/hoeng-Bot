...
const FULL_TAG_DISCORD_LOCALES = new Set(['en-US', 'en-GB', 'zh-TW', 'zh-CN', 'es-ES', 'pt-BR', 'sv-SE']);
 ...
 */
export function buildSlashCommandsWithLocalizations(
    ...
    const processOption = (opt: any): Record<string, any> => {
        ...
        if (opt.minValue !== undefined || opt.min_value !== undefined)
            cleanedOpt.minValue = opt.minValue ?? opt.min_value;
        if (opt.maxValue !== undefined || opt.max_value !== undefined)
            cleanedOpt.maxValue = opt.maxValue ?? opt.max_value;
        if (opt.minLength !== undefined || opt.min_length !== undefined)
            cleanedOpt.minLength = opt.minLength ?? opt.min_length;
        if (opt.maxLength !== undefined || opt.max_length !== undefined)
            cleanedOpt.maxLength = opt.maxLength ?? opt.max_length;
        ...
        if (opt.choices && Array.isArray(opt.choices)) {
            cleanedOpt.choices = opt.choices.map((choice: any) => {
                ...
                return choiceNameLocalizations ? { ...choice, nameLocalizations: choiceNameLocalizations } : choice;
            });
        }
