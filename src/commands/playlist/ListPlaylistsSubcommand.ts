...
export class ListPlaylistsSubcommand extends BasePlaylistSubcommand {
    ...
    public async execute(context: PlaylistSubcommandContext): Promise<void> {
        const playlists = context.playlistManager.getPlaylists(context.command.guild!.id);
        if (playlists.length === 0) {
            await context.command.replySuccess(context.bot, context.command.t('commands:MESSAGE_PLAYLIST_LIST_EMPTY'));
            return;
        }

        await context.command.reply({
            embeds: [embeds.playlistList(context.bot, playlists, context.command.language)],
        });
    }
}
