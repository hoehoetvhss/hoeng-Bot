...
const removeList = (bot: Bot, nowPlaying: string, queueList: string, repeatMode: string, lng?: string) => {
    const embed_ = new EmbedBuilder()
        ...
        .setTitle(bot.i18n.t('embeds:REMOVE_LIST_TITLE', { lng }))
        .addFields({ name: nowPlaying, value: queueList })
        .setTimestamp()
        .setFooter({ text: bot.i18n.t('embeds:REMOVE_LIST_LOOP_MODE', { repeatMode: repeatMode, lng }) });

    return embed_;
};

const removeTrack = (bot: Bot, musicTitle: string, lng?: string) => {
    const embed_ = new EmbedBuilder()
        .setColor(bot.config.bot.embedsColors.success as HexColorString | number)
        .setTitle(bot.i18n.t('embeds:REMOVE_TRACK_TITLE', { lng }))
