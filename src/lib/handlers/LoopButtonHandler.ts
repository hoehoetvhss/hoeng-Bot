...
export class LoopButtonHandler extends DashboardButtonHandler {
    ...
    public static async handle(
        ...
        const select = new StringSelectMenuBuilder()
            ...
            .setOptions(
                this.LOOP_MODE_VALUES.map((value, index) =>
                    new StringSelectMenuOptionBuilder()
                        .setLabel(modeLabels[index])
                        .setDescription(
                            bot.i18n.t('commands:LOOP_SELECT_DESCRIPTION', { mode: modeLabels[index], lng }),
                        )
                        .setValue(value)
            ...
            await i.update({
                embeds: [
                    embeds.textSuccessMsg(
                        bot,
                        bot.i18n.t('events:MESSAGE_SET_LOOP_MODE', { mode: modeLabel, lng: currentLng }),
                    ),
                ],
                components: [],
            });
        ...
        collector.on('end', async (collected: Collection<string, StringSelectMenuInteraction>, reason: string) => {
            if (reason === 'time' && collected.size === 0) {
                ...
                await msg
                    .edit({
                        embeds: [
                            embeds.textErrorMsg(bot, bot.i18n.t('commands:ERROR_TIME_EXPIRED', { lng: currentLng })),
                        ],
                        components: [],
                    })
                    .catch(() => {});
